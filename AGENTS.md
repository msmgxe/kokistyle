...
- **Una sola página por auto-ajuste (ago 2026)**: `buildChangeOrderPdf()` compone la orden con la **rejilla normal** y, si no entra en una página, la vuelve a componer con la **rejilla compacta** (`coGrid(compact)`: fuentes y recuadros ~15% más bajos); se queda con la compacta sólo si de verdad ahorra páginas. Además el cronograma, la nota y las firmas **viajan juntos**: si el cierre no cabe detrás del cronograma, el grupo entero pasa a la hoja siguiente — nunca queda una hoja con sólo las firmas. La cláusula final se mide antes de colocar la firma, así el bloque completo (línea → nombres → fecha → cláusula) siempre cae sobre el pie sin pisarse.
...
- **Subtotal manual por grupo (ago 2026)**: en el resumen, *Subtotal agrega* y *Subtotal acredita* tienen **Fijar / Set** — se escribe el precio del bloque completo sin capturar monto por línea (los inputs de ese grupo se deshabilitan). En el PDF, **ese grupo imprime su alcance sin montos por línea** pero conserva su banda de subtotal con el valor fijado; el otro grupo sigue con sus montos. Se guardan como **líneas centinela `{ kind: "add_total" | "credit_total", amount }` dentro del JSONB `lines`** — no necesitan migración. El monto por línea **nunca se bloquea**: si no hay subtotal fijado, el subtotal es la suma de las líneas. `coGroupTotal(row, kind)` las lee y `editCo`/`coNetOf` las filtran del listado de líneas.
...
- **Cuotas editables (ago 2026)**: el formulario muestra el bloque *Cuotas actualizadas — como salen impresas*, idéntico al del PDF: glosa · %, el importe anterior tachado cuando cambia y el **importe nuevo en un input**. Vacío = el que calcula `coSchedule()`; escrito = manda sobre el recálculo (✕ vuelve al calculado, ↺ Recalcular limpia todas). Si las cuotas dejan de sumar el contrato nuevo, avisa con la diferencia. Se guardan como centinelas `{ kind: "sched", section: "<índice>", amount }` en el JSONB `lines` — sin migración.
- **Vista previa en vivo**: botón *Ver vista previa del PDF* en la vista de armado — iframe con el PDF real que se regenera solo (debounce 400 ms) al editar líneas, montos, motivo o los totales manuales. Mismo patrón que la factura.
...
- API en `src/lib/pdf.ts`: `ChangeOrderData` (con `netOverride`, `addOverride`, `creditOverride`) · `changeOrderTotals()` · `exportChangeOrderPdf()` · `openChangeOrderPdfInBrowser()` · `getChangeOrderPdfBlob()`.
...
| Reporte | `/proyectos/reporte` | Reporte de pendientes (objetivos + notas de varios proyectos) — solo superadmin. Mismo `ReportBuilder` que abre el botón "Reporte" del dashboard: elige proyectos + qué incluir (objetivos/notas, "solo pendientes"), preview en vivo y **vista previa de PDF** (`getPendientesReportBlob` + `PdfPreviewModal`) para imprimir/guardar |
...
- **RLS por proyecto (fase 2, en curso)**: la identidad sale del token (`lux_role`, `lux_projects`) y las políticas se apoyan en tres helpers de Postgres — `lux_role()`, `lux_is_admin()` y `lux_can_see(project_id)`. Scripts por lotes en `sql/fase2-paso*.sql`, cada uno con su comprobación y su marcha atrás; `sql/fase2-verificar.sql` impersona los claims de cada rol dentro de una transacción para probar el aislamiento sin crear usuarios. Reglas ya aplicadas: dinero y documentos internos sólo superadmin (3A); pagos que el cliente lee pero no toca y `project_contacts` cerrado con vista `project_contacts_public` sin montos para el Day Planner (3B); `projects` y sus hijos por `lux_can_see` (3C); materiales, contactos y el estimado de lectura para los del proyecto y escritura sólo del superadmin (3D).
- **Regla de oro al escribir políticas**: nunca `FOR ALL` con un `USING` permisivo — `USING` gobierna también el `DELETE`, así que quien puede ver podría borrar. Una política por comando.
- **Vistas de escape**: una vista corre con los permisos de su dueño y **no hereda el RLS de la tabla base**, así que la autorización va explícita en su `WHERE` (`lux_can_see(...)`). Es el patrón para tapar columnas sensibles, que RLS no filtra.
- **Pendiente (fase 2)**: ocultar `cost`/`profit` de `estimate_items` y `materials.cost` a quien no sea superadmin, sacar `app_users`/`user_project_access` del navegador, retirar las políticas `anon` y pasar el Storage a privado con URLs firmadas.
...
- **Responsive móvil (jul 2026)**: en `<lg` el planner se apila — top bar con `flex-wrap`, pool a ancho completo con scroll propio (max-h 300px) arriba y las columnas de días abajo (scroll horizontal); la página hace el scroll vertical. En `lg+` vuelve al layout lado a lado con alturas fijas.
