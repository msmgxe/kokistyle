...
import { ChevronDown, ChevronUp, Plus, X, Trash2, FileText, Zap, Info } from "lucide-react";
...
export default function EstimateTab({
  ...
  const load = useCallback(async () => {
    ...
    // Try catalog (graceful if tables not yet created)
    try {
      const { data } = await supabase
        .from("estimate_section_catalog")
        .select("*")
        .order("sort_order");
  ...
  const updateItemLocal = (sectionId: string, itemId: string, field: "description" | "amount", value: string) => {
    const parsed = field === "amount" ? (parseFloat(value) || 0) : undefined;
    setEstimate(p => p ? ({
      ...p,
      sections: p.sections.map(s => s.id !== sectionId ? s : {
        ...s,
        items: s.items.map(i => i.id !== itemId ? i : {
          ...i, [field]: field === "amount" ? (parsed ?? 0) : value,
        }),
      }),
    }) : p);
  };

  const saveItemField = useCallback(async (itemId: string, field: "description" | "amount", value: string) => {
  ...
  const itemCount = estimate.sections.reduce((s, sec) => s + sec.items.length, 0);
  ...
  return (
    <div className="space-y-3">
      ...
      {estimate.sections.map(section => {
        ...
        const secTotal = sectionDisplayTotal(section);

        return (
          <div key={section.id} className="overflow-hidden rounded-2xl border border-[#E6DDCB] bg-white">
            {/* Card header */}
            <div
            ...
            >
              <div className="flex min-w-0 items-center gap-3">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm
                  ${section.is_material_type ? "bg-[#FDF0ED]" : "bg-[#EDF3FB]"}`}>
              ...
              <div className="ml-3 flex shrink-0 items-center gap-3">
                ...
                {isOpen
                  ? <ChevronUp size={14} className="text-[#5C6A6E]" />
                  : <ChevronDown size={14} className="text-[#5C6A6E]" />}
              </div>
            </div>
            ...
            {isOpen && (
              <div className="border-t border-[#F0EBE0]">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#F0EBE0] bg-[#FAFAF8] px-4 py-2">
                  <label className="flex cursor-pointer items-center gap-2 text-[11px] text-[#5C6A6E]">
                    <input
                      ...
                      onChange={e => updateSectionField(section.id, "is_material_type", e.target.checked)}
                      className="rounded"
                    />
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={e => updateSectionField(section.id, "note", e.target.value)}
                ...
                {section.items.map(item => (
                  <div
                    key={item.id}
                    className="group flex items-center gap-2 border-b border-[#F0EBE0] px-4 py-2 hover:bg-[#FDFAF6]"
                  >
                    <div className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#C4B89A]" />
                    <input
                      type="text"
                      value={item.description}
                      onChange={e => updateItemLocal(section.id, item.id, "description", e.target.value)}
                      onBlur={e => saveItemField(item.id, "description", e.target.value)}
                    ...
                    <input
                      type="number"
                      value={item.amount || ""}
                      onChange={e => updateItemLocal(section.id, item.id, "amount", e.target.value)}
                      onBlur={e => saveItemField(item.id, "amount", e.target.value)}
                      placeholder="0"
          ...
          <div>
            ...
            <div className="space-y-2">
              {(estimate.deposit_schedule ?? defaultDeposits()).map((dep, i) => {
                const colors = ["bg-[#395886]", "bg-[#4E7A82]", "bg-[#4F8A63]"];
                return (
                  <div key={i} className="flex items-center gap-2.5">
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[11px] font-bold text-white ${colors[i] ?? "bg-[#5C6A6E]"}`}>
                      {dep.pct}%
                    </div>
                    <div>
